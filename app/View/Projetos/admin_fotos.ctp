<h1><strong>Projeto: </strong><?php echo $projeto['Projeto']['titulo']?></h1>
...
<?php
					echo $this->Form->input('projetos_id', array('label' => false, 'type' => 'hidden', 'value' => $projeto['Projeto']['id']));
						 ...
						 echo $this->Form->submit('Inserir imagem', array('class' => 'btn btn-info btn-md')); 
		...
		if(!empty($fotos[0])){
			foreach($fotos as $item):
				if(!empty($item['FotoProjeto']['nome'])){
					...
					echo  $this->Html->image('uploads/projetos/small_'.$item['FotoProjeto']['nome'], array('alt' =>'', 'title' => '', 'class' => 'img-thumbnail'), array('admin' => true));
